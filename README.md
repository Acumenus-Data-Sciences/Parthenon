...
# Edit .env — set a unique PARTHENON_INTERNAL_TOKEN (required) and DB_PASSWORD.
# Edit backend/.env — match DB_PASSWORD to .env, set Sanctum domains.
# (APP_KEY is generated by the key:generate step below.)

# Create the shared external networks (idempotent; the installer does this for you)
docker network create acumenus 2>/dev/null || true
docker network create acropolis-backend 2>/dev/null || true

# Create the bind-mounted host directory for the wiki corpus volume
mkdir -p data/wiki
...
# Build the frontend (the `node` service is in the `dev` profile, so use `run`)
docker compose --profile dev run --rm node sh -c "cd /app && npm ci --legacy-peer-deps && npx vite build"
```

Open **http://localhost:8082** — you'll be greeted by the setup wizard.

> The interactive `python3 install.py` installer performs every step above
> (token generation, networks, host dirs, env, build, seed) automatically and is
> the recommended path. The manual steps are documented for transparency and
> for environments that manage their own orchestration.
...
The CE/EE seams that EE plugs into are documented at [`docs/lineage/design/architecture/extension-points.md`](docs/lineage/design/architecture/extension-points.md). The eight extension points (auth driver, tenant resolver, crypto provider, audit sink, observability shipper, feature flags + EnterpriseGate, installer phase registry, compose composition contract) all shipped in v1.0.7 — see the [v1.0.7 release notes](docs/blog/2026-05-10-v1-0-7-release-notes.md) for the full architectural story.
