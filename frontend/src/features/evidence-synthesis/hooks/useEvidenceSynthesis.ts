...
export function useEvidenceSynthesisAnalyses(
  page?: number,
  search?: string,
  ...
  return useQuery({
    queryKey: ["evidence-synthesis", { page, search, status, scope }],
    queryFn: () => listEvidenceSynthesis({ page, search, status, scope }),
  });
}
