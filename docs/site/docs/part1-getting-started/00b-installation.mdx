...
Both paths use the same source bootstrap and Python installer with a 9-phase guided setup.
...
For a repeatable install, pin the release tag:

```bash
curl -fsSL https://parthenon.acumenus.net/install.sh | sh -s -- --version v1.0.9
```

To work directly from a source checkout:
...
If the installer fails at any phase, re-run `python3 install.py --webapp` from the source checkout. It detects the last completed phase and offers to resume from where it left off.
:::
...
```bash
python3 install.py --webapp --defaults-file my-config.json
```
