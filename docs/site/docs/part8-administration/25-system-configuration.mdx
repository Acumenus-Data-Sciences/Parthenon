...
|----------|---------------|-------------|
| **Ollama** (default) | MedGemma 27B, MedGemma 1.5 4B, Llama 3, Mistral | Local (no API key required) |
...
For healthcare environments without internet access, Ollama runs entirely on-premises with local models. The default Abby configuration uses MedGemma 27B (`puyangwang/medgemma-27b-it:q4_0`), a medical domain-tuned model suitable for clinical research assistance. No data leaves your infrastructure.
:::
