...
> **Confirmed 2026-06-26 (Abby Provider Entitlements plan, shipped).** `ai.frontier`
> gates **API-backed frontier access**, not the existence of Abby. Abby chat/RAG now
> has a capability-driven provider router that defaults to local Ollama/MedGemma and
> supports Anthropic / OpenAI / OpenAI-compatible **API-key** providers behind a
> single safety+budget+audit contract; the subscription/API boundary is enforced
> (`external_subscription_app` profiles are rejected from backend routing). CE keeps
> the full local Abby UX; only the API-key frontier path is the EE/BYO-key entitlement.
> See `docs/lineage/modules/abby-ai/provider-entitlements-and-fallback.md`.
