...
|---|---|---|
| `2026-07-22-omop-vocabulary-refresh-irsf-preservation.md` | The Athena refresh, restore-verified IRSF preservation set, test isolation, staging-first loader, and versioned BGE/Solr/Chroma/Hecate synchronization are complete. The plan remains open because IRSF DQD is below 80%, representative cohort/analysis regeneration is outstanding, and hierarchy/downstream retry orchestration is not yet automated. | Govern the six legacy nonstandard RxNorm brand references and temporal DQD debt; complete representative cohort and analysis read-path acceptance; automate hierarchy/VSAC plus checkpointed downstream refreshes; then close with a retention decision. |
