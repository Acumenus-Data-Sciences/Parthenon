...
You are implementing an end-to-end observational hypertension characterization study inside the Parthenon platform. The study was authored by Glenn H. Bock, MD ("DRAFT: Characterization of hypertension in a large study population", 04/17/2026). Read `docs/lineage/modules/STUDIES.md`, `docs/lineage/modules/analyses/`, and `CLAUDE.md` before writing any code. Query the Parthenon Brain (`parthenon-brain` MCP, collections `parthenon_docs` and `parthenon_code`) for prior art on:
