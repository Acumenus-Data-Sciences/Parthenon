...
Without the CLA, EE cannot legally embed/link CE code under non-AGPL terms. **Heritage authors are not retroactively bound** — the existing audit (per `docs/lineage/handoffs/Apache2.0_to_AGPLv3_Conversion.md`) confirmed no surviving substantial contributions from heritage authors in the current tree.
...
Execute the existing handoff (`docs/lineage/handoffs/Apache2.0_to_AGPLv3_Conversion.md`), with the org rename folded in. Single PR.
