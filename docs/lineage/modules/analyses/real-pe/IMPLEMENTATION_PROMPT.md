...
Read the existing scaffold before you touch anything: [`docs/lineage/modules/analyses/real-pe/README.md`](./README.md). The sub-folders `sql/` and `cohorts/` already contain working starting points — do not rewrite them from scratch; extend them.
...
1. Run `docs/lineage/modules/analyses/real-pe/sql/01_feasibility_counts.sql` against the Acumenus CDM:
   ```bash
   docker compose exec -T postgres psql -U parthenon -d parthenon \
     -v ON_ERROR_STOP=1 \
     -f docs/lineage/modules/analyses/real-pe/sql/01_feasibility_counts.sql \
     | tee docs/lineage/modules/analyses/real-pe/reports/01_feasibility_$(date +%Y%m%d).txt
   ```
...
1. Run `docs/lineage/modules/analyses/real-pe/sql/00_hydrate_concept_ids.sql` and capture the results.
2. For each file in `docs/lineage/modules/analyses/real-pe/cohorts/`, replace `"CONCEPT_ID": 0` with the resolved standard `concept_id`. If a source code is non-standard, use the mapped-to standard concept_id from the second query in the hydration script. Preserve `includeDescendants` and `includeMapped` flags exactly.
...
1. Promote `docs/lineage/modules/analyses/real-pe/sql/02_isth_barc_outcomes.sql` into a proper Parthenon SqlRender template. Target location: `backend/resources/sql/templates/real_pe/isth_barc_outcomes.sql`. Replace the psql bind syntax (`:target_cohort_id`) with SqlRender `@variable` syntax.
...
1. Append a phase-by-phase completion log to `docs/lineage/modules/analyses/real-pe/IMPLEMENTATION_LOG.md` with dates, row counts, and any deviations or escalations.
...
- Scaffold entry point: `docs/lineage/modules/analyses/real-pe/README.md`
- Feasibility SQL: `docs/lineage/modules/analyses/real-pe/sql/01_feasibility_counts.sql`
- Concept hydration SQL: `docs/lineage/modules/analyses/real-pe/sql/00_hydrate_concept_ids.sql`
- Outcome derivation SQL template: `docs/lineage/modules/analyses/real-pe/sql/02_isth_barc_outcomes.sql`
- USCDT target cohort JSON: `docs/lineage/modules/analyses/real-pe/cohorts/01-uscdt-target.json`
- MT comparator cohort JSON: `docs/lineage/modules/analyses/real-pe/cohorts/02-mt-comparator.json`
- ISTH bleed dx-component cohort JSON: `docs/lineage/modules/analyses/real-pe/cohorts/03-isth-major-bleed-dx-component.json`
- ICH outcome cohort JSON: `docs/lineage/modules/analyses/real-pe/cohorts/04-intracranial-hemorrhage.json`
