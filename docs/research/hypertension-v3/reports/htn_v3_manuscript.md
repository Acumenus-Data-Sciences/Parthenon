...
**Methods.** A target cohort (T) of adults aged ≥18 with a first recorded essential-hypertension diagnosis and no prior cardiovascular disease, thyroid disease, secondary hypertension, abnormal kidney function, or antihypertensive exposure was generated on the Acumenus CDM (n = 1,047,308). A normotensive comparator pool (C) was generated using the same exclusion criteria absent the HTN diagnosis (n = 649,278). Cohort eras were extended to 5 years from index (or end of observation, whichever came first). Incidence rates for a major adverse cardiovascular events (MACE) composite (myocardial infarction, ischemic or hemorrhagic stroke, heart failure requiring inpatient encounter) and for incident chronic kidney disease were computed per 1,000 person-years using OHDSI's CohortIncidence package (HADES R, v3.0.0).
...
Four cohorts were operationally defined and locked in design version v2:

| Cohort | Role | Definition | Persons |
|---|---|---|---|
| **T** | Target | Incident essential-hypertension diagnosis (SNOMED 320128 + standard descendants) in adults ≥18 with: no prior cardiovascular disease (composite of MI, stroke, heart failure, peripheral vascular disease), no prior thyroid disease, no prior secondary hypertension, no prior antihypertensive drug exposure (ATC C02–C09 broad scope), no prior diagnosis of abnormal kidney function. Five-year cohort era (EndStrategy.DateOffset = 1825 days from cohort_start) truncated at observation_period_end. | 265,498 |
| **C** | Comparator pool | Adults ≥18 with documented BP measurements but no essential-hypertension diagnosis, otherwise sharing T's full exclusion set. Intended for downstream 1:1 propensity-score matching against T (matching method specified by PI as logistic propensity + caliper 0.2 SD of the logit). | 649,278 |
| **O1** | Outcome | First occurrence of MACE composite components: myocardial infarction (SNOMED 4329847 family), ischemic or hemorrhagic stroke (SNOMED 443454 / 381316 / 432923 / 439847), or heart failure (SNOMED 316139 family). Heart-failure events were subsequently restricted to those with an inpatient `visit_occurrence` qualifier per the PI's pre-execution decision. | 79,506 |
| **O2** | Outcome | First-occurrence diagnosis of chronic kidney disease (SNOMED 46271022 family). Baseline kidney function abnormality was an exclusion criterion in T, so events here represent incident progression. | 160,819 |

Concept sets were materialized to the `concept_sets` table via the Parthenon Study Designer using the OHDSI ATC and SNOMED vocabularies, expanded to standard descendants through `concept_ancestor`. The full materialized concept set inventory comprised 28 sets (IDs 168–195 in `app.concept_sets`).
...
5. **Heart-failure events.** The MACE composite restricts HF to encounters with an inpatient `visit_occurrence` per the PI's pre-execution decision. Some genuine HF events outside an inpatient context are therefore not counted, which may underestimate the MACE rate.
