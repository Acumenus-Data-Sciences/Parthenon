...
- [x] **Phase 19: UA County Urban/Rural Stratification** - Deploy dormant `gis` schema (5 tables + matview) under `parthenon_migrator` ownership with HIGHSEC GRANTs; load Census Bureau 2020 UA county data (3,234 rows) + HUD ZIP↔county crosswalk; ship env-DSN multi-source loaders (`load_ua_county.py`, `load_geography.py`, `load_crosswalk.py`, `loader_common.py`) covering ACUMENUS / PANCREAS / IRSF (synthetic-zip CDMs SYNPUF/EUNOMIA excluded via D-07 allow-list); skip CT_2022 sheet (D-06); UPSERT into `gis.external_exposure` keyed by `(source_id, person_id, exposure_type, exposure_date)` (D-01/D-05); wire `location_urban_pct` as the FIRST urban/rural Studies stratification covariate via `IncidenceRateService::buildIncidenceRateSql` with query-time bucketing (`<0.25 Highly Rural / <0.50 Rural / <0.75 Mixed / ≥0.75 Urban` per D-03); IncidenceRateDesigner exposes Location dropdown with Pancreas care-site warning (D-08); each loader creates paired `app.gis_imports` row for GIS Panel reconciliation; closes GIS-04 by remediating 7 legacy loaders + tightening DSN regression guard (12/12 PASS, no `dbname=ohdsi`/`password=acumenus`/`user=smudoshi` literals). Live DEV smoke on ACUMENUS cohort 77 returned 4 monotonic urban/rural buckets totaling 253,952 identified persons. (completed 2026-04-27)
