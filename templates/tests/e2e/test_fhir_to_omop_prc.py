"""E2E: fhir_to_omop PR-C (DiagnosticReport + Consent on top of PR-A + PR-B).

Extends Plan 6's PR-B E2E with the two PR-C resource types. Seeds the
consent permit/deny concepts and the LOINC panel concept the
DiagnosticReport fixture references; creates the ``app.consent_decisions``
audit table inline (Phase 0 ships only ``app.runs``); asserts post-load
row counts:

  - PERSON = 2, VISIT_OCCURRENCE = 2 (PR-A unchanged)
  - CONDITION_OCCURRENCE = 2, MEASUREMENT = 2 (PR-A unchanged)
  - OBSERVATION = 5 (2 social-history PR-A + 1 DiagnosticReport + 2 Consent)
  - PROCEDURE_OCCURRENCE = 1, DRUG_EXPOSURE = 2 (PR-B unchanged)
  - app.consent_decisions = 2, with co2 recording a 'deny' decision
"""
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                ...
                "(40757491, 'Lipid panel', 'Measurement', 'LOINC', 'Lab Test', "
                "'S', '24331-1', '1970-01-01', '2099-12-31'), "
                "(4055893, 'Patient consent given', 'Observation', 'SNOMED', "
                "'Procedure', 'S', '1234-PER', '1970-01-01', '2099-12-31'), "
                "(4054745, 'Patient consent withdrawn', 'Observation', 'SNOMED', "
                "'Procedure', 'S', '1234-DEN', '1970-01-01', '2099-12-31')"
            )
        )
        # Phase 0 ships only person + concept + a barebones drug_exposure;
        # recreate clinical tables and the PR-C audit table inline.
        ...
        for ddl in (
            ...
            """,
            """
            CREATE TABLE IF NOT EXISTS app.consent_decisions (
...
def test_fhir_to_omop_prc_runs_to_completion(
    ...
    with PostgresContainer("postgres:16") as pg:
        ...
        bootstrap(version="5.4", schema="app", engine=engine)
        ...
        r = client.post(
            ...
            json={
                ...
                "version": "0.1.0",
                "parameters": params,
                "correlation_id": "fhir-to-omop-prc-e2e",
            },
            headers=_auth(),
        )
        ...
        with engine.connect() as conn:
            ...
            obs_dr_consent = conn.execute(
                text(
                    "SELECT COUNT(*) FROM omop.observation WHERE observation_type_concept_id = 32856"
                )
            ).scalar()
            consent_decisions = conn.execute(
                text("SELECT COUNT(*) FROM app.consent_decisions")
            ).scalar()
            co2_decision = conn.execute(
                text("SELECT decision FROM app.consent_decisions WHERE consent_id = 'co2'")
            ).scalar()
        ...
        # PR-A 2 social-history + PR-C 1 DiagnosticReport + 2 Consent = 5
        assert observations == 5, f"expected 5 observations, got {observations}"
        assert procedures == 1, f"expected 1 procedure, got {procedures}"
        assert drugs == 2, f"expected 2 drug_exposures, got {drugs}"
        assert (
            obs_dr_consent == 3
        ), f"expected 3 OBSERVATION rows of type 32856, got {obs_dr_consent}"
        assert consent_decisions == 2, f"expected 2 consent_decisions rows, got {consent_decisions}"
        assert co2_decision == "deny", f"expected co2 decision='deny', got {co2_decision!r}"
