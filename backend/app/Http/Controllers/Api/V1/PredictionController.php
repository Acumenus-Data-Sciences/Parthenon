...
class PredictionController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $payload['counts'] = [
                'active' => PredictionAnalysis::query()->where('author_id', $userId)->where('status', 'active')->count(),
                'draft' => PredictionAnalysis::query()->withoutGlobalScope(LibraryDefaultScope::class)
                    ->where('author_id', $userId)->where('status', 'draft')->count(),
                'archived' => PredictionAnalysis::query()->withoutGlobalScope(LibraryDefaultScope::class)
                    ->where('author_id', $userId)->where('status', 'archived')->count(),
                'all' => PredictionAnalysis::query()->withoutGlobalScope(LibraryDefaultScope::class)
