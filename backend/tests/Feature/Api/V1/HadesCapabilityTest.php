...
it('proxies the HADES package capability inventory from Darkstar', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertJsonPath('data.parity_status', 'stale')
        ->assertJsonPath('data.required_missing_count', 0)
        ->assertJsonPath('data.required_outdated_count', 1)
        ->assertJsonPath('data.shiny_policy.expose_hosted_surfaces', true)
        ->assertJsonPath('data.shiny_policy.allow_iframe_embedding', true)
        ->assertJsonPath('data.shiny_policy.allow_user_supplied_app_paths', false)
        ->assertJsonPath('data.shiny_policy.decision', 'managed_ohdsi_shiny_compatibility')
        ->assertJsonPath('data.shiny_apps.0.key', 'plp-results')
        ...
        ->assertJsonPath('data.packages.0.target_version', '6.0.2')
        ->assertJsonPath('data.packages.0.version_status', 'behind')
        ->assertJsonPath('data.packages.1.installed', false)
        ->assertJsonPath('data.packages.2.surface', 'managed_shiny_compatibility')
        ->assertJsonPath('data.packages.2.priority', 'high')
        ->assertJsonPath('data.packages.2.required_for_parity', true)
        ->assertJsonPath('data.packages.2.hosted_surface', true)
        ->assertJsonPath('data.packages.2.exposure_policy', 'managed_compatibility_layer')
        ->assertJsonPath('data.packages.2.decision', 'managed_ohdsi_shiny_compatibility')
        ->assertJsonPath('data.packages.3.surface', 'managed_shiny_compatibility')
        ->assertJsonPath('data.packages.3.priority', 'high')
        ->assertJsonPath('data.packages.3.required_for_parity', true)
        ->assertJsonPath('data.packages.3.hosted_surface', true)
        ->assertJsonPath('data.packages.3.exposure_policy', 'managed_compatibility_layer')
        ->assertJsonPath('data.packages.3.decision', 'managed_ohdsi_shiny_compatibility');
});
