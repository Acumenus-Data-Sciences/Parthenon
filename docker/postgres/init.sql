...
CREATE SCHEMA IF NOT EXISTS omop;          -- OMOP CDM clinical tables
CREATE SCHEMA IF NOT EXISTS vocab;         -- Shared OMOP Vocabulary tables
...
-- Grants
GRANT ALL ON SCHEMA app, omop, vocab, results, gis, eunomia, eunomia_results, php, webapi TO parthenon;
...
DO $$
BEGIN
    EXECUTE format('GRANT CREATE ON DATABASE %I TO parthenon', current_database());
END
$$;
